...
import { CursorState } from '@upstream/engine/cursor';
...
import { DeleteSelectionCommand, ApplyCharFormatCommand, SnapshotCommand } from '@upstream/engine/command';
import type { OperationDescriptor } from '@upstream/engine/command';
...
export class InputHandler {
  ...
  private formBboxToOverlayRect(bbox: { x: number; y: number; w: number; h: number }, pageIdx: number): { left: number; top: number; width: number; height: number } {
    ...
    const pageLeft = resolvePageLeft(this.virtualScroll.getPageLeft(pageIdx), contentWidth, pageDisplayWidth);
